...
def preload_images(scene_count: int, width: int, height: int) -> None:
    """预加载所有图像到缓存"""
    def load_single_image(scene_number: int):
        image_path = f"assets/images/Tomato_{scene_number:05d}_.png"
...
def get_cached_image(image_path: str, width: int, height: int) -> np.ndarray:
    """获取缓存的图像"""
    with image_cache_lock:
        if image_path in image_cache:
            return image_cache[image_path].copy()  # 返回副本避免多线程冲突
...
def render_frame_batch(frame_batch_info: List[Tuple[int, float]], 
    ...
    for frame_idx, current_time in frame_batch_info:
        ...
        # 加载对应场景的图片（使用缓存）
        image_path = f"assets/images/Tomato_{scene_number:05d}_.png"
        img = get_cached_image(image_path, width, height)
...
def load_audio_clips_concurrent(audio_paths: List[str], scene_durations: Dict[int, float]) -> List[AudioFileClip]:
    """并发加载音频文件"""
    print("并发加载音频文件...")
    ...
    for scene in sorted(scene_durations.keys()):
        audio_path = f"assets/audios/{scene:02d}.wav"
...
def create_complete_video():
    ...
    # 生成完整的RST字幕文件
    audio_files = [f"assets/audios/{i:02d}.wav" for i in range(1, len(captions) + 1)]
    complete_rst_path = "assets/subtitles/complete_video.rst"
    ...
    # 预加载图像资源
    scene_count = len(set(seg['scene'] for seg in all_segments))
    preload_images(scene_count, width, height)
    ...
    # 并发加载配音
    voice_clips = load_audio_clips_concurrent(audio_files, scene_durations)
    ...
    if voice_clips or bg_music:
        try:
            ...
            # 直接保存最终视频
            final_output_path = "videos/complete_video_with_audio.mp4"
        ...
        print("没有音频内容，创建无声视频")
        final_output_path = "videos/complete_video.mp4"
        os.makedirs("videos", exist_ok=True)
